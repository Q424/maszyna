...
TModel3d* TMdlContainer::LoadModel(static char *newName, bool dynamic)
{
...
TModel3d* TModelsManager::LoadModel(static char *Name, bool dynamic)
{
...
// CZY BEDZIE OK?
TModel3d *TModelsManager::GetModel(static char *Name, bool dynamic)
{ // model mo¿e byæ we wpisie
	// "node...model" albo
	// "node...dynamic", a tak¿e
	// byæ dodatkowym w dynamic
	// (kabina, wnêtrze, ³adunek)
	// dla "node...dynamic" mamy podan¹ œcie¿kê w "\dynamic\" i musi byæ co
	// najmniej 1 poziom, zwkle s¹ 2
	...
	if (strchr(Name, '\\') == NULL) {
		...
		if (strchr(Name, '/') != NULL) {
			...
			Global::asCurrentTexturePath.erase(
				Global::asCurrentTexturePath.find("/") + 1,Global::asCurrentTexturePath.length());
		}
	}
	else {
		strcpy(buf, Name);
		if (dynamic) // na razie tak, bo nie wiadomo, jaki mo¿e mieæ wp³yw na
			...
			if (strchr(Name, '/') != NULL) { // pobieranie tekstur z katalogu, w którym jest model
				Global::asCurrentTexturePath =
					Global::asCurrentTexturePath + std::string(Name);
				Global::asCurrentTexturePath.erase(Global::asCurrentTexturePath.find("/") + 1, Global::asCurrentTexturePath.length());
			}
	}
