...
// friction.cpp is equal with 1166
...
double TP10Bg::GetFC(double N, double Vel) 
{
 ...
 return exp(-0.022*N)*(0.19 - 0.095*exp(-Vel / 25.7)) + 0.384*exp(-Vel / 25.7) - 0.028;
}
...
return exp(-0.017*N)*(0.18 - 0.09*exp(-Vel / 25.7)) + 0.381*exp(-Vel / 25.7) - 0.022;//0.05*exp(-0.2*N);
}
...
{
 return 0.60*((1.6*N + 100) / (8.0*N + 100))*((Vel + 100) / (5 * Vel + 100));
