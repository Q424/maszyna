 ...
 switch (iType)
 {
  ...
  case TP_EVLAUNCH:
   if (EvLaunch->Render())
    if ((EvLaunch->dRadius<0)||(mgn<EvLaunch->dRadius))
    {
     WriteLogSS("Eventlauncher "+asName, "INFO");
...
{//wyœwietlanie obiektu przez Display List
 ...
 switch (iType)
 {
  ...
  case TP_MODEL:
   return Model->RenderDLq(&pCenter); //!
 }
...
{//finalna kwalifikacja na listy renderowania, jeœli nie obs³ugiwane grupowo
 ...
 {case TP_TRACK:
  ...
  case TP_SUBMODEL: //submodele terenu w kwadracie kilometrowym id¹ do nRootMesh
   //WriteLog("nRootMesh was "+std::string(nRootMesh?"not null ":"null ")+IntToHex(int(this),8));
 ...
 *parser >> r >> rmin;
 parser->getTokens();
 *parser >> token;
 asNodeName=std::string(token.c_str());
 parser->getTokens();
 *parser >> token;
 str=std::string(token.c_str());
 TGroundNode *tmp,*tmp2;
 tmp=new TGroundNode();
 tmp->asName=(asNodeName==std::string("none")?std::string(""):asNodeName);
...
// else if (str=="isolated")            tmp->iType=TP_ISOLATED;
 ...
 if (bError)
 {
  Error(std::string("Scene parse error near "+str).c_str());
  for (int i=0;i<60;++i)
 ...
 switch (tmp->iType)
 {
   ...
   *parser >> token;
    ...
    *parser >> token; //wypada³o by to zapamiêtaæ...
    tmp->hvTraction->asParallel=std::string(token.c_str());
    parser->getTokens();
   ...
   *parser >> tmp->pCenter.x >> tmp->pCenter.y >> tmp->pCenter.z;
   ...
   {//dodanie do wyszukiwarki
    ...
    {//przy zdublowaniu wskaŸnik zostanie podmieniony w drzewku na póŸniejszy (zgodnoœæ wsteczna)
     Error("Duplicated memcell: "+tmp->asName); //to zg³aszaæ duplikat
    }
    else
   ...
   {//dodanie do wyszukiwarki
    ...
    {//przy zdublowaniu wskaŸnik zostanie podmieniony w drzewku na póŸniejszy (zgodnoœæ wsteczna)
     if (tmp->pTrack->iCategoryFlag&1) //jeœli jest zdublowany tor kolejowy
      Error("Duplicated track: "+tmp->asName); //to zg³aszaæ duplikat
    }
    else
   ...
   *parser >> token;
   str=std::string(token.c_str());
   tmp->tsStaticSound->Init(stdstrtochar(str), sqrt(tmp->fSquareRadius), tmp->pCenter.x, tmp->pCenter.y, tmp->pCenter.z, false, rmin);
...
//            tmp->iState=(Parser->GetNextSymbol().LowerCase()=="loop"?DSBPLAY_LOOPING:0);
  ...
  case TP_DYNAMIC:
   ...
   parser->getTokens();
   *parser >> token;
   str1=std::string(token.c_str()); //katalog
   //McZapkie: doszedl parametr ze zmienialna skora
   parser->getTokens();
   *parser >> token;
   Skin=std::string(token.c_str()); //tekstura wymienna
   parser->getTokens();
   *parser >> token;
   str3=std::string(token.c_str()); //McZapkie-131102: model w MMD
   if (bTrainSet)
    ...
    *parser >> tf1; //Ra: -1 oznacza odwrotne wstawienie, normalnie w sk³adzie 0
    parser->getTokens();
    *parser >> token;
    DriverType=std::string(token.c_str()); //McZapkie:010303 - w przyszlosci rozne konfiguracje mechanik/pomocnik itp
    tf3=fTrainSetVel; //prêdkoœæ
    parser->getTokens();
    *parser >> token;
    str4=std::string(token.c_str());
    ...
    if (int2>0) //yB: jesli znalazl kropke, to ja przetwarza jako parametry
    {
      ...
      int1=str4.substr(1, int2-1).ToInt(); //niech sprzegiem bedzie do kropki cos
...
//    *parser >> int1; //yB: nastawy i takie tam TUTAJ!!!!!
   ...
   {//pojazd wstawiony luzem
    ...
    parser->getTokens();
    *parser >> token;
    str=std::string(token.c_str()); //track
    parser->getTokens();
    *parser >> tf1; //Ra: -1 oznacza odwrotne wstawienie
    parser->getTokens();
    *parser >> token;
    DriverType=std::string(token.c_str()); //McZapkie:010303: obsada
    parser->getTokens();
   ...
   *parser >> int2; //iloœæ ³adunku
   ...
   {//je¿eli ³adunku jest wiêcej ni¿ 0, to rozpoznajemy jego typ
    parser->getTokens();
    *parser >> token;
    str2=std::string(token.c_str());  //LoadType
    if (str2==std::string("enddynamic")) //idiotoodpornoœæ: ³adunek bez podanego typu
    {
   ...
   {//jeœli tor znaleziony
    ...
    //WriteLog("Dynamic shift: "+std::string(fTrainSetDist));
   ...
   {//gdy tor nie znaleziony
    Error("Missed track: dynamic placed on \""+tmp->DynamicObject->asTrack+"\"");
    delete tmp;
    tmp=NULL; //nie mo¿e byæ tu return, bo trzeba pomin¹æ jeszcze enddynamic
   }
   parser->getTokens();
   *parser >> token;
    ...
    *parser >> token;
    if (tmp)
     tmp->DynamicObject->asDestination=std::string(token.c_str());
    *parser >> token;
   }
...
/*q
   ...
   {//dodanie do wyszukiwarki
    ...
    {//przy zdublowaniu wskaŸnik zostanie podmieniony w drzewku na póŸniejszy (zgodnoœæ wsteczna)
     Error("Duplicated model: "+tmp->asName); //to zg³aszaæ duplikat
    }
    else
...
//    tf=Parser->GetNextSymbol().ToDouble();
   //          TempVerts[i].tu=tf;
     //        tf=Parser->GetNextSymbol().ToDouble();
      ...
      Error("Bad triangles: too many verices");
     i++;
     parser->getTokens();
     *parser >> token;
...
TEvent* TGround::FindEventScan(const std::string &asEventName)
{//wyszukanie eventu z opcj¹ utworzenia niejawnego dla komórek skanowanych
 ...
 if (asEventName.substr(asEventName.length()-4,5)==":scan") //jeszcze mo¿e byæ event niejawny
 {//no to szukamy komórki pamiêci o nazwie zawartej w evencie
  std::string n=asEventName.substr(1,asEventName.length()-5); //do dwukropka
   ...
   {//pojazdów w ogóle nie dotyczy dodawanie do mapy
    ...
    {//specjalne przetwarzanie terenu wczytanego z pliku E3D
     ...
     {//od 1 do koñca s¹ zestawy trójk¹tów
      xxxzzz=std::string(Current->nNode[j].smTerrain->pName); //pobranie nazwy
//-- gr=GetRect(1000*(xxxzzz.substr(1,3).ToIntDef(0)-500),1000*(xxxzzz.substr(4,3).ToIntDef(0)-500));
      if (Global::bUseVBO)
...
//    else if ((Current->iType!=GL_TRIANGLES)&&(Current->iType!=GL_TRIANGLE_STRIP)?true //~czy trójk¹t?
 ...
 GlobalTime->Init(hh, mm, srh, srm, ssh, ssm); //McZapkie-300302: inicjacja czasu rozkladowego - TODO: czytac z trasy!
...
{//g³ówne wczytywanie scenerii
 //if (asFile.LowerCase().substr(1,7)=="scenery")
 // asFile.erase(1,8); //Ra: usuniêcie niepotrzebnych znaków - zgodnoœæ wstecz z 2003

 WriteLog(std::string("Loading scenery from "+asFile).c_str());
 Global::pGround=this;
 //pTrain=NULL;
 ...
 std::string token;
    ...
    while (token!="") //(!Parser->EndOfFile)
    {
     ...
     {
      refresh=0;
     //-- Global::DoEvents(); // TODO:
     }
     else ++refresh;
     str=std::string(token.c_str());
     if (str==std::string("node"))
     {
      ...
      {//je¿eli przetworzony poprawnie
       ...
       if (LastNode) //dopiero na koniec dopisujemy do tablic
        if (LastNode->iType != TP_DYNAMIC) //jeœli nie jest pojazdem...
        {
      ...
      {
       Error("Scene parse error near "+std::string(token.c_str()));
       //break;
      }
     }
	 /*q
     else
     if (str==std::string("trainset"))
     {
      iTrainSetWehicleNumber=0;
      nTrainSetNode=NULL;
      ...
      parser.getTokens();
      parser >> token;
      asTrainName=std::string(token.c_str());  //McZapkie: rodzaj+nazwa pociagu w SRJP
      parser.getTokens();
      parser >> token;
      asTrainSetTrack=std::string(token.c_str()); //œcie¿ka startowa
      parser.getTokens(2);
      parser >> fTrainSetDist >> fTrainSetVel; //przesuniêcie i prêdkoœæ
     }
     else
     if (str==std::string("endtrainset"))
     ...
     else if (str==std::string("event"))
     {
      ...
      {//najpierw sprawdzamy, czy nie ma, a potem dopisujemy
       ...
       {//jeœli znaleziony duplikat
        ...
        else if (i>8?tmp->asName.substr(1,9)=="lineinfo:":false) //tymczasowo wyj¹tki
        {delete tmp; tmp=NULL;} //tymczasowa utylizacja duplikatów W5
        else if (i>8?tmp->asName.substr(i-7,8)=="_warning":false) //tymczasowo wyj¹tki
        {delete tmp; tmp=NULL;} //tymczasowa utylizacja duplikatu z tr¹bieniem
        else if (i>4?tmp->asName.substr(i-3,4)=="_shp":false) //nie podlegaj¹ logowaniu
        ...
        if (tmp) //jeœli nie zosta³ zutylizowany
         ...
         {
		  Error("Duplicated event: "+tmp->asName);
          found->Append(tmp); //doczepka (taki wirtualny multiple bez warunków)
       ...
       {//jeœli nie duplikat
        ...
        {//jeœli nazwa wyst¹pi³a, to do kolejki i wyszukiwarki dodawany jest tylko pierwszy
         if (RootEvent->Type!=tp_Ignored)
          if (RootEvent->asName.find("onstart")) //event uruchamiany automatycznie po starcie
...
//     else
//     if (str==std::string("include"))  //Tolaris to zrobil wewnatrz parsera
//     {
//         Include(Parser);
//     }
     else if (str==std::string("rotate"))
     {
      //parser.getTokens(3);
      ...
      //WriteLog("*** rotate "+std::string(aRotate.x)+" "+std::string(aRotate.y)+" "+std::string(aRotate.z));
     }
     else if (str==std::string("origin"))
     {
//      str=Parser->GetNextSymbol().LowerCase();
//      if (str=="begin")
      {
       ...
       {
        MessageBox(0,std::string("Origin stack overflow ").c_str(),"Error",MB_OK);
        break;
       }
       parser.getTokens(3);
     ...
     else if (str==std::string("endorigin"))
     {
//      else
  //    if (str=="end")
      {
       if (OriginStackTop<=0)
       {
        MessageBox(0,std::string("Origin stack underflow ").c_str(),"Error",MB_OK);
        break;
       }
     ...
     else if (str==std::string("atmo"))   //TODO: uporzadkowac gdzie maja byc parametry mgly!
     ...
     else if (str==std::string("sky"))
     {//youBy - niebo z pliku
      WriteLog("Scenery sky definition");
      parser.getTokens();
      parser >> token;
      std::string SkyTemp;
      SkyTemp=std::string(token.c_str());
     ...
     else if (str==std::string("firstinit"))
      FirstInit();
     else if (str==std::string("description"))
     {
      do
      {
       parser.getTokens();
     ...
     {//pomijanie od nierozpoznanej komendy do jej zakoñczenia
      ...
      {//jeœli nie liczba, to spróbowaæ pomin¹æ komendê
       WriteLog(std::string("Unrecognized command: "+str).c_str());
       str="end"+str;
       do
       {
        parser.getTokens();
       ...
       Error(std::string("Unrecognized command: "+str));
     }
     else
     if (str==std::string(""))
         break;
  ...
  switch (Current->Type)
  {
   ...
   case tp_UpdateValues: //zmiana wartoœci
    ...
    {//McZapkie-100302
     ...
     {//jeœli chodzi o zajetosc toru (tor mo¿e byæ inny, ni¿ wpisany w komórce)
      ...
      if (!Current->Params[9].asTrack)
       Error("Bad event: track \""+std::string(Current->asNodeName)+"\" does not exists in \""+Current->asName+"\"");
     }
     ...
     if (!tmp->MemCell->asTrackName.empty()) //tor powi¹zany z komórk¹ powi¹zan¹ z eventem
     {//tu potrzebujemy wskaŸnik do komórki w (tmp)
      ...
      if (trk)
       ...
       Error("Bad memcell: track \""+tmp->MemCell->asTrackName+"\" not exists in memcell \""+tmp->asName+"\"");
     }
     else
    ...
    {//nie ma komórki, to nie bêdzie dzia³a³ poprawnie
     ...
     Error("Bad event: \""+Current->asName+"\" cannot find memcell \""+Current->asNodeName+"\"");
    }
   break;
   ...
   case tp_CopyValues: //skopiowanie komórki do innej
    ...
    if (tmp)
    {
     ...
     Error("Bad copyvalues: event \""+Current->asName+"\" cannot find memcell \""+Current->asNodeName+"\"");
    ...
    if (tmp)
    {
     ...
     Error("Bad copyvalues: event \""+Current->asName+"\" cannot find memcell \""+std::string(buff)+"\"");
   break;
   case tp_Animation: //animacja modelu
    ...
    if (tmp)
    {
     ...
     {//standardowo przypisanie submodelu
      ...
      {
		  //TODO:q zakomentowalem tymczasowo
       ...
       //- Current->Params[9].asAnimContainer->EventAssign(FindEvent(Current->asNodeName+"."+std::string(buff)+":done"));
      }
     }
    }
    else
     Error("Bad animation: event \""+Current->asName+"\" cannot find model \""+Current->asNodeName+"\"");
    Current->asNodeName="";
   break;
   case tp_Lights: //zmiana œwiete³ modelu
    ...
    if (tmp)
     ...
     Error("Bad lights: event \""+Current->asName+"\" cannot find model \""+Current->asNodeName+"\"");
    Current->asNodeName="";
   break;
   case tp_Visible: //ukrycie albo przywrócenie obiektu
    ...
    if (tmp)
     ...
     Error("Bad visibility: event \""+Current->asName+"\" cannot find model \""+Current->asNodeName+"\"");
    Current->asNodeName="";
   break;
    ...
    {//dowi¹zanie toru
     ...
     if (!Current->Params[0].asInt) //jeœli prze³¹cza do stanu 0
      if (Current->Params[2].asdouble>=0.0) //jeœli jest zdefiniowany dodatkowy ruch iglic
       //Current->Params[9].asTrack->Switch(0); //przes³anie parametrów
	     Current->Params[9].asTrack->Switch(0, Current->Params[1].asdouble, Current->Params[2].asdouble);
    }
    else
     Error("Bad switch: event \""+Current->asName+"\" cannot find track \""+Current->asNodeName+"\"");
    Current->asNodeName="";
   break;
   case tp_Sound: //odtworzenie dŸwiêku
    ...
    if (tmp)
     ...
     Error("Bad sound: event \""+Current->asName+"\" cannot find static sound \""+Current->asNodeName+"\"");
    Current->asNodeName="";
   break;
   case tp_TrackVel: //ustawienie prêdkoœci na torze
    if (!Current->asNodeName.empty())
    {
     ...
     {tmp->pTrack->iAction|=0x200; //flaga zmiany prêdkoœci toru jest istotna dla skanowania
      ...
      Error("Bad velocity: event \""+Current->asName+"\" cannot find track \""+Current->asNodeName+"\"");
    }
    Current->asNodeName= "";
   break;
   ...
   case tp_Multiple:
    ...
    {//jeœli chodzi o zajetosc toru
     ...
     if (!Current->Params[9].asTrack)
     {Error(std::string("Bad event: Track \"")+std::string(buff)+"\" does not exist in \""+Current->asName+"\"");
    ...
    {//jeœli chodzi o komorke pamieciow¹
     ...
     {Error(std::string("Bad event: MemCell \"")+std::string(buff)+std::string("\" does not exist in \""+Current->asName+"\""));
    ...
    for (i=0;i<8;i++)
    {
     if (Current->Params[i].asText!=NULL)
     {
      ...
      if (!Current->Params[i].asEvent) //Ra: tylko w logu informacja o braku
       if (std::string(Current->Params[i].asText).substr(1,5)!="none_")
       {
		WriteLog(std::string(std::string("Event \"")+std::string(buff)+std::string("\" does not exist")).c_str());
        Error("Missed event: "+std::string(buff)+" in multiple "+Current->asName);
       }
     }
    }
   break;
   case tp_Voltage: //zmiana napiêcia w zasilaczu (TractionPowerSource)
    if (!Current->asNodeName.empty())
    {
     ...
     if (tmp)
      ...
      Error("Bad voltage: event \""+Current->asName+"\" cannot find power source \""+Current->asNodeName+"\"");
    }
    Current->asNodeName= "";
   break;
...
{//³¹czenie torów ze sob¹ i z eventami
 ...
 for (Current=nRootOfType[TP_TRACK];Current;Current=Current->nNext)
 {
  ...
  if (Current->asName.substr(1,1)=="*") //mo¿liwy portal, jeœli nie pod³¹czony od striny 1
   if (!Track->CurrentPrev()&&Track->CurrentNext())
    Track->iCategoryFlag|=0x100; //ustawienie flagi portalu
	*/
 }
 //WriteLog("Total "+std::string(tracks)+", far "+std::string(tracksfar));
  ...
  if (nTemp) //jak zasilacz znaleziony
   ...
   if (Traction->asPowerSupplyName != "*") //gwiazdka dla przês³a z izolatorem
    ...
    {//logowanie b³êdu i utworzenie zasilacza o domyœlnej zawartoœci
	 ...
	 //TODO: Q zakomentowalem tymczasowo
     Error("Missed TractionPowerSource: "+Traction->asPowerSupplyName);
  ...
  if (!Traction->hvNext[0]) //tylko jeœli jeszcze nie pod³¹czony
  {
   ...
   if (Traction->hvNext[0]) //jeœli zosta³ pod³¹czony
    if (Traction->psSection&&tmp->psSection) //tylko przês³o z izolatorem mo¿e nie mieæ zasilania, bo ma 2, trzeba sprawdzaæ s¹siednie
     ...
     //-  Error("Bad power: at "+FloatToStrF(Traction->pPoint1.x,ffFixed,6,2)+" "+FloatToStrF(Traction->pPoint1.y,ffFixed,6,2)+" "+FloatToStrF(Traction->pPoint1.z,ffFixed,6,2));
     }
  }
   ...
   if (Traction->hvNext[1]) //jeœli zosta³ pod³¹czony
    if (Traction->psSection&&tmp->psSection) //tylko przês³o z izolatorem mo¿e nie mieæ zasilania, bo ma 2, trzeba sprawdzaæ s¹siednie
     ...
     {//to mo¿e byæ albo pod³¹czenie podstacji lub kabiny sekcyjnej do sekcji, albo b³¹d
      ...
      //-else //jeœli obie to sekcje albo obie podstacje, to bêdzie b³¹d
      //- Error("Bad power: at "+FloatToStrF(Traction->pPoint2.x,ffFixed,6,2)+" "+FloatToStrF(Traction->pPoint2.y,ffFixed,6,2)+" "+FloatToStrF(Traction->pPoint2.z,ffFixed,6,2));
     }
  }
 }
  ...
  if (!Traction->asParallel.empty()) //bêdzie wskaŸnik na inne przês³o
   ...
   {nTemp=FindGroundNode(Traction->asParallel,TP_TRACTION);
    ...
    {//o ile zostanie znalezione przês³o o takiej nazwie
     if (!nTemp->hvTraction->hvParallel) //jeœli tamten jeszcze nie ma wskaŸnika bie¿ni wspólnej
      Traction->hvParallel=nTemp->hvTraction; //wpisaæ siebie i dalej daæ mu wskaŸnik zwrotny
     else //a jak ma, to albo do³¹czyæ siê do kó³eczka
      Traction->hvParallel=nTemp->hvTraction->hvParallel; //przj¹æ dotychczasowy wskaŸnik od niego
     nTemp->hvTraction->hvParallel=Traction; //i na koniec ustawienie wskaŸnika zwrotnego
    }
    if (!Traction->hvParallel)
     Error("Missed overhead: "+Traction->asParallel); //logowanie braku
   }
...
bool TGround::InitLaunchers()
{
 ...
 for (Current=nRootOfType[TP_EVLAUNCH];Current;Current=Current->nNext)
 {
  ...
  if (EventLauncher->iCheckMask!=0)
   if (EventLauncher->asMemCellName!=std::string("none"))
  ...
  EventLauncher->Event1=(EventLauncher->asEvent1Name!=std::string("none"))?FindEvent(EventLauncher->asEvent1Name):NULL;
  EventLauncher->Event2=(EventLauncher->asEvent2Name!=std::string("none"))?FindEvent(EventLauncher->asEvent2Name):NULL;
 }
 return true;
}
    ...
    {//teraz mog¹ byæ warunki do tych eventów
     ...
     {//McZapkie-100302 - updatevalues oprocz zmiany wartosci robi putcommand dla wszystkich 'dynamic' na danym torze
      ...
      if (Global::DebugModeFlag)
       WriteLogSS("EVENT EXECUTED: AddValues & Track command - "+std::string(Event->Params[0].asText) +" "+ ftoss(Event->Params[1].asdouble) +" "+ ftoss(Event->Params[2].asdouble), "INFO");
     }
     else
      if (Global::DebugModeFlag)
       WriteLogSS("EVENT EXECUTED: AddValues - "+std::string(Event->Params[0].asText) +" "+ ftoss(Event->Params[1].asdouble) +" "+ ftoss(Event->Params[2].asdouble), "INFO");
    }
    Event=Event->evJoined; //jeœli jest kolejny o takiej samej nazwie, to idzie do kolejki
   }
   if (Event)
   {//standardowe dodanie do kolejki
    WriteLogSS("EVENT ADDED TO QUEUE: "+Event->asName+(Node?std::string(" by "+Node->asName):std::string("")), "INFO");
    Event->fStartTime=fabs(Event->fDelay)+Timer::GetTime(); //czas od uruchomienia scenerii
    if (Event->fRandomDelay>0.0)
     Event->fStartTime+=Event->fRandomDelay* 5000 *0.0001; //doliczenie losowego czasu opóŸnienia
...
{//sprawdzenie spelnienia warunków dla eventu
 ...
 else if (e->iFlags&conditional_propability)
 {
  ...
  WriteLogSS("Random integer: "+ftoss(rprobability) +"/"+ ftoss(e->Params[10].asdouble), "INFO");
 ...
 {//porównanie wartoœci
  if (tmpEvent->Params[9].asMemCell->Compare(e->Params[10].asText,e->Params[11].asdouble,e->Params[12].asdouble,e->iFlags))
   return true;
  else if (Global::iWriteLogEnabled && Global::DebugModeFlag)
  {//nie zgadza siê, wiêc sprawdzmy, co

   //--LogComment=e->Params[9].asMemCell->Text()+std::string(" ") +FloatToStrF(e->Params[9].asMemCell->Value1(),ffFixed,8,2)+" "+FloatToStrF(tmpEvent->Params[9].asMemCell->Value2(),ffFixed,8,2)+" != ";
  /*q
...
/*
 ...
 {//wypisanie kolejki
  ...
  while (tmpEvent)
  {
   WriteLog(tmpEvent->asName+" "+std::string(tmpEvent->fStartTime));
   tmpEvent=tmpEvent->Next;
  }
 }
*/
 ...
 {//eventy s¹ posortowana wg czasu wykonania
  ...
  {//w zasadzie te wy³¹czone s¹ skanowane i nie powinny siê nigdy w kolejce znaleŸæ
   WriteLogSS("EVENT LAUNCHED: "+tmpEvent->asName+(tmpEvent->Activator?std::string(" by "+tmpEvent->Activator->asName):std::string("")), "INFO");
   switch (tmpEvent->Type)
   {
    ...
    case tp_UpdateValues:
     ...
     {//teraz mog¹ byæ warunki do tych eventów
      ...
      {//McZapkie-100302 - updatevalues oprocz zmiany wartosci robi putcommand dla wszystkich 'dynamic' na danym torze
       ...
       if (Global::DebugModeFlag)
        WriteLogSS("Type: UpdateValues & Track command - "+std::string(tmpEvent->Params[0].asText) +" "+ ftoss(tmpEvent->Params[1].asdouble) +" "+ ftoss(tmpEvent->Params[2].asdouble), "INFO");
      }
      else
       if (Global::DebugModeFlag)
        WriteLogSS("Type: UpdateValues - "+std::string(tmpEvent->Params[0].asText) +" "+ ftoss(tmpEvent->Params[1].asdouble) +" "+ ftoss(tmpEvent->Params[2].asdouble), "INFO");
     }
    break;
    case tp_GetValues:
    ...
    case tp_TrackVel:
     ...
     {//prêdkoœæ na zwrotnicy mo¿e byæ ograniczona z góry we wpisie, wiêkszej siê nie ustawi eventem
	  ...
	  if (Global::DebugModeFlag) //wyœwietlana jest ta faktycznie ustawiona
	...
	*/
    case tp_LogValues: //zapisanie zawartoœci komórki pamiêci do logu
     if (tmpEvent->Params[9].asMemCell) //jeœli by³a podana nazwa komórki
      WriteLogSS("Memcell \""+tmpEvent->asNodeName+"\": "+
       tmpEvent->Params[9].asMemCell->Text() +" "+
       ftoss(tmpEvent->Params[9].asMemCell->Value1()) +" "+
       ftoss(tmpEvent->Params[9].asMemCell->Value2()), "INFO");
     else //lista wszystkich
      for (TGroundNode *Current=nRootOfType[TP_MEMCELL];Current;Current=Current->nNext)
       WriteLogSS("Memcell \""+Current->asName+"\": "+
        Current->MemCell->Text() +" "+
        ftoss(Current->MemCell->Value1()) +" "+
        ftoss(Current->MemCell->Value2()), "INFO");
    break;
    case tp_Voltage: //zmiana napiêcia w zasilaczu (TractionPowerSource)
...
{//aktualizacja animacji krokiem FPS: dt=krok czasu [s], dt*iter=czas od ostatnich przeliczeñ
 ...
 {//pierwsza iteracja i wyznaczenie stalych:
  ...
  for (TGroundNode *Current=nRootDynamic;Current;Current=Current->nNext)
   Current->DynamicObject->Update(dt,dt1); //Ra 2015-01: tylko tu przelicza sieæ trakcyjn¹
 }
 else
...
{//aktualizacja drutu zasilaj¹cego dla ka¿dego pantografu, ¿eby odczytaæ napiêcie
   ...
   {//je¿eli znamy drut z poprzedniego przebiegu
    ...
    {//powtarzane a¿ do znalezienia odpowiedniego odcinka na liœcie dwukierunkowej
     ...
     {//jeœli t jest w przedziale, wyznaczyæ odleg³oœæ wzd³u¿ wektorów vUp i vLeft
      ...
      fHorizontal=fabs(DotProduct(vGdzie,vLeft))-p->fWidth; //to siê musi mieœciæ w przedziale zale¿nym od szerokoœci pantografu
      ...
      {//po wyselekcjonowaniu drutu, przypisaæ go do toru, ¿eby nie trzeba by³o szukaæ
       ...
       //WriteLog("Drut: "+std::string(fHorizontal)+" "+std::string(fVertical));
         ...
         {//jeœli tylko jest w przedziale, wyznaczyæ odleg³oœæ wzd³u¿ wektorów vUp i vLeft
          ...
          if (fVertical>=0.0) //jeœli ponad pantografem (bo mo¿e ³apaæ druty spod wiaduktu)
           ...
           else if (fVertical<p->PantTraction) //ale ni¿ej, ni¿ poprzednio znaleziony
           {
            ...
            {//problem jest, gdy nowy drut jest wy¿ej, wtedy pantograf od³¹cza siê od starego, a na podniesienie do nowego potrzebuje czasu
             fVertical+=0.15*fHorizontal/p->fWidthExtra; //korekta wysokoœci o nabie¿nik - drut nad nabie¿nikiem jest geometrycznie jakby nieco wy¿ej
             if (fVertical<p->PantTraction) //gdy po korekcie jest ni¿ej, ni¿ poprzednio znaleziony
 ...
 //{//nieu¿ywana normalnie zmienna ogranicza powtórzone logowania
  ...
  //ErrorLog(FloatToStrF(1000.0*model->MoverParameters->DistCounter,ffFixed,7,3)+","+FloatToStrF(p->PantTraction,ffFixed,7,3)+","+FloatToStrF(p->fHorizontal,ffFixed,7,3)+","+FloatToStrF(p->PantWys,ffFixed,7,3)+","+std::string(p->hvPowerWire?1:0)); //
...
//---------------------------------------------------------------------------
void TGround::TerrainRead(const std::string &f)
