...
TEventLauncher::TEventLauncher() { // ustawienie początkowych wartości dla
  ...
  *parser >> token;
  ...
  if (str != "none") {
	  if (str.length() == 1)
		  ...
		  iKey = stoi(str); // str.ToIntDef(0); // a jak więcej, to jakby numer klawisza jest
  }
  parser->getTokens();
  *parser >> DeltaTime;
  if (DeltaTime < 0)
  ...
  parser->getTokens();
  *parser >> token;
  asEvent1Name = std::string(token.c_str()); // pierwszy event
  parser->getTokens();
  *parser >> token;
  asEvent2Name = std::string(token.c_str()); // drugi event
  if ((asEvent2Name == "end") ||
      (asEvent2Name == "condition")) { // drugiego eventu może nie być, bo są z
  ...
  if (str == std::string("condition")) { // obsługa wyzwalania warunkowego
    parser->getTokens();
    *parser >> token;
    asMemCellName = std::string(token.c_str());
    parser->getTokens();
    *parser >> token;
...
bool TEventLauncher::Render() { //"renderowanie" wyzwalacza
  ...
  { // jeśli nie cykliczny, to sprawdzić czas
    if (GlobalTime->hh == iHour) {
      if (GlobalTime->mm == iMinute) { // zgodność czasu uruchomienia
        if (UpdatedTime < 10) {
...
bool TEventLauncher::IsGlobal() { // sprawdzenie, czy jest globalnym wyzwalaczem
                                  // czasu
  if (DeltaTime == 0)
    if (iHour >= 0)
      if (iMinute >= 0)
