...
TEvent::TEvent(std::string m) {
  // asName=""; //czy nazwa eventu jest niezbędna w tym przypadku? chyba nie
  evNext = evNext2 = NULL;
  bEnabled = false; // false dla eventów używanych do skanowania sygnałów (nie
                    // dodawane do kolejki)
  ...
  Type = m.empty() ? tp_Unknown : tp_GetValues; // utworzenie niejawnego
                                                  // odczytu komórki pamięci w
                                                  // torze
  for (int i = 0; i < 13; i++)
    Params[i].asPointer = NULL;
  evJoined = NULL; // nie ma kolejnego z tą samą nazwą, usuwane są wg listy
                   // Next2
  Activator = NULL;
  iFlags = 0;
  // event niejawny jest tworzony przed fazą InitEvents, która podmienia nazwę
  // komórki pamięci na wskaźnik
...
TEvent::~TEvent() {
  switch (Type) { // sprzątanie
  case tp_Multiple:
    // SafeDeleteArray(Params[9].asText); //nie usuwać - nazwa obiektu
    // powiązanego zamieniana na wskaźnik
    if (iFlags &
        conditional_memstring) // o ile jest łańcuch do porównania w memcompare
  ...
  case tp_Animation: // nic
    // SafeDeleteArray(Params[9].asText); //nie usuwać - nazwa jest zamieniana
    // na wskaźnik do submodelu
...
void TEvent::Conditions(cParser *parser, std::string s) { // przetwarzanie warunków, wspólne dla Multiple i UpdateValues
  if (s == "condition") { // jesli nie "endevent"
    std::string token;
    std::string str;
    if (!asNodeName.empty()) { // podczepienie łańcucha, jeśli nie jest pusty
      Params[9].asText =
          new char[asNodeName.length() + 1]; // usuwane i zamieniane na wskaźnik
    ...
    *parser >> token;
    ...
    if (str == "trackoccupied")
      ...
      *parser >> token;
      ...
      {
        Params[12].asdouble = stof(str);
        iFlags |= conditional_memval2;
      }
    }
    parser->getTokens();
    *parser >> token;
    s = token.c_str(); // ewentualnie dalej losowe opóźnienie
  }
  if (s == "randomdelay") { // losowe opóźnienie
    std::string token;
    parser->getTokens();
...
void TEvent::Load(cParser *parser, vector3 *org) {
  ...
  parser->getTokens();
  *parser >> token;
  asName = token.c_str(); // użycie parametrów może dawać wielkie TODO: zmienic na male!!!
  ...
  *parser >> token;
  ...
  if (str == "exit")
    Type = tp_Exit;
  else if (str == "updatevalues")
    Type = tp_UpdateValues;
  else if (str == "getvalues")
    Type = tp_GetValues;
  else if (str == "putvalues")
    Type = tp_PutValues;
  else if (str == "disable")
    Type = tp_Disable;
  else if (str == "sound")
    ...
    Type = tp_DynVel;
  else if (str == "trackvel")
    Type = tp_TrackVel;
  else if (str == "multiple")
    Type = tp_Multiple;
  else if (str =="addvalues")
    Type = tp_AddValues;
  else if (str == "copyvalues")
    Type = tp_CopyValues;
  else if (str == "whois")
    Type = tp_WhoIs;
  else if (str == "logvalues")
    Type = tp_LogValues;
  else if (str == "voltage")
  ...
  *parser >> token;
  ...
  case tp_CopyValues:
    ...
    iFlags =
        update_memstring | update_memval1 | update_memval2; // normalanie trzy
    i = 0;
    parser->getTokens();
    *parser >> token; // nazwa drugiej komórki (źródłowej)
    while (token.compare("endevent") != 0) {
      switch (++i) { // znaczenie kolejnych parametrów
      case 1: // nazwa drugiej komórki (źródłowej)
        Params[9].asText =
            new char[token.length() + 1]; // usuwane i zamieniane na wskaźnik
        strcpy(Params[9].asText, token.c_str());
        break;
      case 2: // maska wartości
       //- iFlags =
       //-     AnsiString(token.c_str())
  ...
  case tp_PutValues:
    parser->getTokens(3);
    *parser >> Params[3].asdouble >> Params[4].asdouble >>
        Params[5].asdouble; // położenie X,Y,Z
    if (org) { // przesunięcie
    ...
    *parser >> token;
    str = token.c_str();
    if (str.substr(1, 19) == "PassengerStopPoint:") {
      if (str.find("#"))
        str = str.substr(1, str.find("#") - 1); // obcięcie unikatowości
      bEnabled = false; // nie do kolejki (dla SetVelocity też, ale jak jest do
                        // toru dowiązany)
    ...
    *parser >> token;
    ...
    if (str == "none")
      ...
      try {
        ...
        Params[1].asdouble = 0.0;
        WriteLogSS("Error: number expected in PutValues event, found: " + str, "ERROR");
      }
    parser->getTokens();
    *parser >> token;
    str = token.c_str();
    ...
    do {
      ...
      *parser >> token;
      if (token.compare("endevent") != 0) {
        str = token.c_str();
        if (i < 8)
          Params[i].asdouble = stof(str); // teraz może mieć ułamek
        i++;
      }
    } while (token.compare("endevent") != 0);
    break;
  ...
  case tp_Velocity:
    ...
    *parser >> token;
    str = token.c_str();
    Params[0].asdouble = stof(str) * 0.28;
    parser->getTokens();
    *parser >> token;
    break;
  case tp_Sound:
    ...
    // - do poprawy!
    parser->getTokens();
    *parser >> Params[0].asInt; // 0: wylaczyc, 1: wlaczyc; -1: wlaczyc
                                // zapetlone
    parser->getTokens();
    *parser >> token;
    break;
    ...
    *parser >> token;
    ...
    if (token.compare("rotate") == 0) { // obrót względem osi
      ...
      *parser >> token;
      ...
      Params[0].asInt = 1;
      parser->getTokens(4);
      *parser >> Params[1].asdouble >> Params[2].asdouble >>
          Params[3].asdouble >> Params[4].asdouble;
  ...
  case tp_Switch:
    ...
    *parser >> token;
    str = token.c_str();
    if (str != "endevent") {
      Params[1].asdouble = stof(str); // prędkość liniowa ruchu iglic
      parser->getTokens();
      *parser >> token;
      str = token.c_str();
    } else
    ...
    if (str != "endevent") {
      Params[2].asdouble = stof(str); // dodatkowy ruch drugiej iglicy
                                           // (zamknięcie nastawnicze)
      parser->getTokens();
      *parser >> token;
    } else
  ...
  case tp_Multiple:
    ...
    *parser >> token;
    str = token.c_str();
    while (str != "endevent" && str != "condition" && str != "randomdelay") {
      if ((str.substr(1, 5) != "none_")
              ? (i < 8)
              : false) { // eventy rozpoczynające się od "none_" są ignorowane
        if (str != "else") {
        ...
        Error("Bad event: \"" + str + "\" ignored in multiple \"" + asName +  "\"!");
      else
        WriteLogSS("Event \"" + str + "\" ignored in multiple \"" + asName + "\"!", "INFO");
      parser->getTokens();
      *parser >> token;
      str = token.c_str();
    }
  ...
  case tp_Unknown: // nieznany
    ...
    WriteLogSS("Bad event: \"" + asName + (Type == tp_Unknown ? "\" has unknown type." : "\" is ignored."), "INFO");
    break;
  }
};

void TEvent::AddToQuery(TEvent *e) { // dodanie eventu do kolejki
  if (evNext ? (e->fStartTime >= evNext->fStartTime) : false)
    evNext->AddToQuery(e); // sortowanie wg czasu
  else { // dodanie z przodu
    e->evNext = evNext;
    evNext = e;
  }
}
...
std::string TEvent::CommandGet() { // odczytanie komendy z eventu
  switch (Type) { // to się wykonuje również składu jadącego bez obsługi
  case tp_GetValues:
...
TCommandType TEvent::Command() { // odczytanie komendy z eventu
  switch (Type) { // to się wykonuje również dla składu jadącego bez obsługi
  case tp_GetValues:
...
double TEvent::ValueGet(int n) { // odczytanie komendy z eventu
...
vector3 TEvent::PositionGet() { // pobranie współrzędnych eventu
  switch (Type) { //
  case tp_GetValues:
...
void TEvent::Append(TEvent *e) { // doczepienie kolejnych z tą samą nazwą
  if (evJoined)
    evJoined->Append(
        e); // rekurencja! - góra kilkanaście eventów będzie potrzebne
  else {
    evJoined = e;
