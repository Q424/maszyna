...
*/
// traction.cpp is equal with 1166
// TODO: traction.cpp
...
//    glTranslatef(pPosition.x,pPosition.y,pPosition.z);
  ...
  if (Wires!=0)
  {
      ...
      {
       glBegin(GL_LINE_STRIP);
           glVertex3f(float(pPoint3.x), float(pPoint3.y), float(pPoint3.z));
           for (int i=0; i<iNumSections-1; i++)
           {
               pt3= pPoint3+v1*f;
               t= (1-fabs(f-mid)*2);
              if ((Wires<4)||((i!=0)&&(i!=iNumSections-2))) 
               glVertex3f(float(pt3.x), float(pt3.y-sqrt(t)*fHeightDifference), float(pt3.z));
               f+= step;
           }
           glVertex3f(float(pPoint4.x), float(pPoint4.y), float(pPoint4.z));
       glEnd();
       }
       ...
       glBegin(GL_LINES);
           for (int i=0; i<iNumSections-1; i++)
           {
              ...
              flo1=(Wires==4?+0.05:0); 
               ...
               if ((i%2) == 0)
               {
               glVertex3f(float(pt3.x), float(pt3.y-sqrt(t)*fHeightDifference-((i==0)||(i==iNumSections-2)?flo:flo1)), float(pt3.z));
               glVertex3f(float(pt4.x-(pPoint2.z/ddp-pPoint1.z/ddp)*WireOffset), float(pt4.y), float(pt4.z-(-pPoint2.x/ddp+pPoint1.x/ddp)*WireOffset));
               }
               else
               {
...
// if ((Vert-old)!=iLines)
//  WriteLogSS("!!! Wygenerowano punktów "+std::string(Vert-old)+", powinno byæ "+std::string(iLines), "INFO");
};
...
{//(this) jest przês³em zasilanym, o rezystancji (r), policzyæ rezystancjê zastêpcz¹ s¹siednich
 ...
 {//pod¹¿anie we wskazanym kierunku
  ...
  if (Global::DebugModeFlag) //tylko podczas testów
