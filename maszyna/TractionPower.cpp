...
// tractionpower.cpp is equal with 1166
// TODO: tractionpower.cpp
...
{//powinno byæ wykonane raz na krok fizyki
 if (NominalVoltage*TotalPreviousAdmitance>MaxOutputCurrent) //iloczyn napiêcia i admitancji daje pr¹d
 {
  ...
  {SlowFuse=true;
  Error("Power overload: \"" + gMyNode->asName + "\" disabled for " + ntoss(SlowFuseTimeOut) + "s");
  }
  else
   Error("Power overload: \""+gMyNode->asName+"\" disabled for "+ ntoss(FastFuseTimeOut)+"s");
  FuseTimer=0;
 }
