...
{//pobranie obiektu trajektorii ruchu
 ...
 parser->getTokens();
 *parser >> token;
 str=std::string(token.c_str()); //typ toru
...
//    iDamageFlag=Parser->GetNextSymbol().ToInt();   //damage
 ...
 parser->getTokens();
 *parser >> token;
 str=std::string(token.c_str());  //environment
 if (str=="flat")
  eEnvironment=e_flat;
 ...
 *parser >> token;
 ...
 if (bVisible)
 {
  parser->getTokens();
  *parser >> token;
  str=std::string(token.c_str());   //railtex
  ...
  *parser >> fTexLength; //tex tile length
  ...
  parser->getTokens();
  *parser >> token;
  str=std::string(token.c_str());   //sub || railtex
  WriteLog(str.c_str());
 ...
 *parser >> token;
 ...
 while (str!="endtrack")
 {
  if (str=="event0")
  {
   parser->getTokens();
   *parser >> token;
   asEvent0Name=std::string(token.c_str());
  }
  else if (str=="event1")
  {
   parser->getTokens();
   *parser >> token;
   asEvent1Name=std::string(token.c_str());
  }
  else if (str=="event2")
  {
   parser->getTokens();
   *parser >> token;
   asEvent2Name=std::string(token.c_str());
  }
  else if (str=="eventall0")
  {
   parser->getTokens();
   *parser >> token;
   asEventall0Name=std::string(token.c_str());
  }
  else if (str=="eventall1")
  {
   parser->getTokens();
   *parser >> token;
   asEventall1Name=std::string(token.c_str());
  }
  else if (str=="eventall2")
  {
   parser->getTokens();
   *parser >> token;
   asEventall2Name=std::string(token.c_str());
  }
  ...
  {//obwód izolowany, do którego tor nale¿y
   parser->getTokens();
   *parser >> token;
   pIsolated=TIsolated::Find(std::string(token.c_str()));
  }
  else if (str=="angle1")
   ...
   *parser >> token;
   ...
   Error("Unknown property: \""+str+"\" in track \""+name+"\"");
 ...
 if (!pIsolated)
  if ((i=name.find("@"))>0)
   ...
   {pIsolated=TIsolated::Find(name.substr(i+1,name.length()));
    name=name.substr(1,i-1); //usuniêcie z nazwy
   }
}
...
bool TTrack::AssignEvents(TEvent *NewEvent0,TEvent *NewEvent1,TEvent *NewEvent2)
{
 ...
 if (!evEvent0)
 {
  ...
  else
  {
   ...
   {
    Error(std::string("Bad track: Event0 \"")+asEvent0Name+std::string("\" does not exist"));
    bError=true;
   }
  }
 }
 ...
 {
  Error(std::string("Bad track: Event0 cannot be assigned to track, track already has one"));
  bError=true;
 }
 if (!evEvent1)
 {
  ...
  {//Ra: tylko w logu informacja
   Error(std::string("Bad track: Event1 \"")+asEvent1Name+std::string("\" does not exist").c_str());
   bError=true;
  }
 }
 else
 {
  Error(std::string("Bad track: Event1 cannot be assigned to track, track already has one"));
  bError=true;
 }
 if (!evEvent2)
 {
  ...
  {//Ra: tylko w logu informacja
   Error(std::string("Bad track: Event2 \"")+asEvent2Name+std::string("\" does not exist"));
   bError=true;
  }
 }
 else
 {
  Error(std::string("Bad track: Event2 cannot be assigned to track, track already has one"));
  bError=true;
 }
 return !bError;
}
...
bool TTrack::AssignallEvents(TEvent *NewEvent0,TEvent *NewEvent1,TEvent *NewEvent2)
{
 ...
 if (!evEventall0)
 {
  ...
  else
  {
   if (!asEvent0Name.empty())
   {
    Error(std::string("Eventall0 \"")+asEventall0Name+std::string("\" does not exist"));
    bError=true;
   }
  }
 }
 ...
 {
  Error(std::string("Eventall0 cannot be assigned to track, track already has one"));
  bError=true;
 }
 if (!evEventall1)
 {
  ...
  else
  {
   if (!asEvent0Name.empty())
   {//Ra: tylko w logu informacja
    WriteLogSS(std::string("Eventall1 \"")+asEventall1Name+std::string("\" does not exist"), "INFO");
    bError=true;
   }
  }
 }
 ...
 {
  Error(std::string("Eventall1 cannot be assigned to track, track already has one"));
  bError=true;
 }
 if (!evEventall2)
 {
  ...
  else
  {
   if (!asEvent0Name.empty())
   {//Ra: tylko w logu informacja
    WriteLogSS(std::string("Eventall2 \"")+asEventall2Name+std::string("\" does not exist"), "INFO");
    bError=true;
   }
  }
 }
 ...
 {
  Error(std::string("Eventall2 cannot be assigned to track, track already has one"));
  bError=true;
 }
 return !bError;
}
...
{//ustawienie wskaŸnika na segement w po¿¹danym kierunku (into) od strony (from)
 ...
 switch (into)
 {case 0: //stop
   //WriteLog("Crossing from P"+std::string(from+1)+" into stop on "+pMyNode->asName);
  break;
  case 1: //left
   //WriteLog("Crossing from P"+std::string(from+1)+" to left on "+pMyNode->asName);
   i=(SwitchExtension->iRoads==4)?iLewo4[from]:iLewo3[from];
  break;
  case 2: //right
   //WriteLog("Crossing from P"+std::string(from+1)+" to right on "+pMyNode->asName);
   i=(SwitchExtension->iRoads==4)?iPrawo4[from]:iPrawo3[from];
  break;
  case 3: //stright
   //WriteLog("Crossing from P"+std::string(from+1)+" to straight on "+pMyNode->asName);
   i=(SwitchExtension->iRoads==4)?iProsto4[from]:iProsto3[from];
  break;
 }
 if (i)
 {Segment=SwitchExtension->Segments[abs(i)-1];
  //WriteLog("Selected segment: "+std::string(abs(i)-1));
 }
 return i;
};
 ...
 if (eType==tt_Cross)
  for (i=0;i<2;++i)
  {
   if (SwitchExtension->pPrevs[i])
    WriteLog("Point "+std::string(i+i+1)+" -> track "+SwitchExtension->pPrevs[i]->pMyNode->asName+":"+std::string(int(SwitchExtension->iPrevDirection[i])));
   if (SwitchExtension->pNexts[i])
    WriteLog("Point "+std::string(i+i+2)+" -> track "+SwitchExtension->pNexts[i]->pMyNode->asName+":"+std::string(int(SwitchExtension->iNextDirection[i])));
  }
  */
};
