...
public:
	...
	//--inline void SetdMoveLen(double dMoveLen) { MoverParameters->dMoveLen = dMoveLen; }
	//--inline void ResetdMoveLen() { MoverParameters->dMoveLen = 0; }
	//--inline double GetdMoveLen() { return MoverParameters->dMoveLen; }
	...
	inline vector3 HeadPosition() { return vCoulpler[iDirection ^ 1]; }; //pobranie wspó³rzêdnych czo³a
	inline vector3 RearPosition() { return vCoulpler[iDirection]; }; //pobranie wspó³rzêdnych ty³u
...
//--	inline double GetWidth() { return MoverParameters->Dim.W; };
	...
	int DirectionGet() { return iDirection + iDirection - 1; }; //odczyt kierunku w sk³adzie
	...
	int RouteWish(TTrack *tr); //zapytanie do AI, po którym segmencie skrzy¿owania jechaæ
	void DestinationSet(std::string to);
